...
- Gemini 是完整 research-agenda workflow 的发散 greenhouse provider；没有 Gemini CLI 或未登录时，基础 vault、`kb_search.py`、KB 审计和模板 idea 仍可用，但这是降级路径，不是完整工作流。Gemini 输出只能进入 `projects/research-agenda/divergent/` 和待审阅 seed/greenhouse 流程，不得直接晋升为成熟 idea 或论文结论。
- OpenCode / DeepSeek 是 Gemini greenhouse 后的强制敌对审稿层；`gemini-divergent` 日常 idea 要 clean success，必须有 Gemini-DeepSeek battle 成功。DeepSeek 负责攻击 A+B 拼接、弱机制、最强 baseline kill 和 lab-fit 风险；失败时必须标 `partial`。
- Codex 是 seed review / second-pass reviewer；它读取 greenhouse、DeepSeek battle 和本地 evidence packet，只能给 accept/rewrite/park/reject-with-rescue，不得自动把 idea 晋升为论文结论。
...
- Gemini is the divergent greenhouse provider for the full research-agenda workflow. If Gemini CLI is not installed or not authenticated, the base vault, local search, audits, and template ideas still work, but that is a degraded path rather than the full workflow. Gemini outputs may only become reviewable candidates in `projects/research-agenda/divergent/` and seed/greenhouse review; they are not mature ideas or paper claims until evidence, similar-work, experiment, risk, and human approval gates pass.
- OpenCode / DeepSeek is the mandatory adversarial review layer after Gemini greenhouse. A clean `gemini-divergent` daily idea stage requires the Gemini-DeepSeek battle to succeed. DeepSeek attacks A+B composition, weak mechanisms, strongest-baseline kill paths, and lab-fit risks; failure must be reported as `partial`.
- Codex is the seed-review / second-pass reviewer. It reads greenhouse output, DeepSeek battle results, and local evidence packets, then classifies candidates as accept/rewrite/park/reject-with-rescue. It must not auto-promote paper claims.
