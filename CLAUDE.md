...
- Gemini 是可选的自动发散 idea provider；没有 Gemini CLI 或未登录时，基础 vault、`kb_search.py`、KB 审计和模板 idea 仍可用。Gemini 输出只能进入 `projects/research-agenda/idea_bank/` 的待审阅 seed/greenhouse 流程，不得直接晋升为成熟 idea 或论文结论。
...
- Gemini is an optional automated divergent-idea provider. If Gemini CLI is not installed or not authenticated, the base vault, local search, audits, and template ideas still work. Gemini outputs may only become reviewable seeds in `projects/research-agenda/idea_bank/`; they are not mature ideas or paper claims until evidence, similar-work, experiment, risk, and human approval gates pass.
