...
- 你仍然需要在自己的机器上配置 Claude/Codex/Gemini/OpenCode DeepSeek/Zotero 登录或环境变量。
...
如果你要启用更完整的 Zotero、arXiv、Gemini、OpenCode/DeepSeek 或 Codex 自动化，请先理解对应命令会读写哪些目录，再在自己的本地配置中手动放宽权限。不要把本机高权限 allow list 提交回公开仓库。
