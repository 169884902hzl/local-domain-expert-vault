...
Purpose:
- Gemini is part of the full automatic research-agenda workflow through `gemini-divergent`.
- This command remains as a manual fallback or debugging path: it prepares a broad, Gemini-friendly prompt file that the user can run manually.
...
Rules:
- Do not treat this manual prompt command as the main daily automation path; the full path is `research_agenda_update.py --idea-generator gemini-divergent` followed by OpenCode / DeepSeek battle and Codex review.
