...
def render_prompt(*, run_date: str, candidates_path: Path | None, items: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "## Prompt",
        "",
        "你是我的研究 idea 发散伙伴，也是一个非常严厉的 Physical AI / robotics PhD advisor。研究背景是机械臂具身操作论文研究，核心轴包括：VLA/VLM/RL-token/action interface，tactile/force/contact-rich manipulation，Sim-to-Real/robustness/failure recovery，以及 DLO/bimanual manipulation。",
        "",
        ...
        "",
        "特别注意：这四条是核心论文研究轴，不是必须机械拼接的关键词。不要泛泛写 RL Token、VLA、触觉或 Sim-to-Real；只有当证据或工程病灶真的指向某条轴时才使用它，并说明它改变了哪个接口、反馈路径、鲁棒性机制或 DLO/双臂测试对象。",
        "",
        "发散时请优先从论文级机器人系统问题出发：VLA action/head 边界不清、RL-token 信息压缩丢失、触觉/力觉反馈无法进入正确控制空间、接触不稳定、双臂互相遮挡、低频 diffusion action chunk 追不上快速接触、DLO 拓扑变化不可观测、Sim-to-Real 接触模型失真、failure recovery 没有强基线、evaluation metric 掩盖失败模式等。你可以借鉴这些例子，但不要复制。",
        "",
