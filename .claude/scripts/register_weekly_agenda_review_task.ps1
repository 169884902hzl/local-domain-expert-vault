...
$WrapperPath = Join-Path $ScriptPath "run_weekly_agenda_review_task.ps1"
...
try {
  ...
  $Day = $DayOfWeek.Substring(0, 3).ToUpperInvariant()
  ...
  $Result = & schtasks.exe /Create /TN $TaskName /SC WEEKLY /D $Day /ST $Time /TR $TaskRun /F 2>&1
